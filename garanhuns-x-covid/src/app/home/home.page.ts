  ...
  isGaranhuns(){
    this.http.get("https://api.ipify.org?format=json").subscribe((ip) => {
      ...
      this.http.get("http://ip-api.com/json/" + Object(ip).ip).subscribe((ipInfo) => {
        console.log({ latitude: -8.9365336, longitude: -36.6418746}); //Centro de Garanhuns
        console.log({ latitude: Object(ipInfo).lat, longitude: Object(ipInfo).lon }); //Coordenadas do usuário)
        ...
        if (isInCity == true){
          console.log("Está em Garanhuns.");
        }
        else{
          console.log("Não está em Garanhuns.");
        }
      });
    });
  }
